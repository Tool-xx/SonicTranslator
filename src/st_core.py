...
_ARTIFACT_PATTERNS = [
    re.compile(r"^\s*GPT-\d+(\.\d+)?o?\s*nano\s*$", re.IGNORECASE),  # covers GPT-4 nano, GPT-4o nano, GPT-4.1 nano
    re.compile(r"^\s*GPT-\d+(\.\d+)?o?\s*$", re.IGNORECASE),  # GPT-4o, GPT-4.1 etc.
    re.compile(r"^\s*Claude.*", re.IGNORECASE),
    re.compile(r"^\s*Llama.*", re.IGNORECASE),
    re.compile(r"^\s*Mixtral.*", re.IGNORECASE),
...
def clean_translation(raw: str, original: str) -> str:
    ...
    The algorithm:
    1. Drop known artifact lines (model labels, UI buttons, banners).
    2. Drop lines that are an exact copy of the source text.
    3. If the source text appears as a substring inside the cleaned output
       (model echoing it), trim everything before it.
    4. If step 3 would leave nothing, return the cleaned output as-is
       (better to return something with a possible echo than to return empty).
    """
    if not raw:
        return ""
    ...
    # Safety: only strip if the result after stripping is non-trivially
    # shorter than the original cleaned text (i.e. we actually removed a
    # meaningful prefix, not just the entire response).
    orig = original.strip()
    if orig and len(orig) > 20:
        match = re.search(re.escape(orig), text, re.IGNORECASE)
        if match and match.start() < len(text) // 2:
            after = text[match.end():].strip()
            if after and len(after) > len(orig) * 0.3:
                text = after
...
def wait_for_response(page, prompt: str) -> str:
    ...
    Uses two selectors with fallback:
    1. ``div[data-activeresponse="true"]``  (current Duck.ai)
    2. ``.markdown`` inside the last assistant message  (fallback)
    """
    SELECTORS = [
        'div[data-activeresponse="true"]',
        '.markdown',
    ]
    ...
    for sel in SELECTORS:
        try:
            page.wait_for_selector(sel, timeout=RESPONSE_TIMEOUT_MS)
            active_selector = sel
            break
        except Exception:
            continue
    ...
    # Extract the text portion of the prompt for echo-detection.
    # The full prompt includes instructions/few-shot examples — we only
    # need the user-supplied text that would actually appear on screen.
    prompt_lines = prompt.strip().splitlines()
    prompt_tail = prompt_lines[-1] if prompt_lines else prompt
    ...
    for _ in range(MAX_STABILITY_POLLS):
        ...
        try:
            count = page.locator(active_selector).count()
            if count == 0:
                continue
            container = page.locator(active_selector).nth(count - 1)
            current = container.inner_text().strip()
            if not current:
                continue
            # Skip containers that only echo the user's input.
            if current == prompt or current == prompt_tail:
                continue
            # Also skip if the response is just the original text repeated.
            # (Duck.ai sometimes shows the user's message in a response-like div.)
            if current.lower().rstrip() == prompt_tail.lower().rstrip():
                continue
...
class TranslationSession:
    ...
    def _translate_once(self, text: str, target_lang: str) -> str:
        ...
        if not clean:
            # Dump the raw response so the user can see what Duck.ai returned.
            try:
                debug_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")
                with open(os.path.join(debug_dir, "debug_raw_response.txt"), "w", encoding="utf-8") as f:
                    f.write("=== RAW RESPONSE FROM DUCK.AI ===\n")
                    f.write(response)
                    f.write("\n\n=== ORIGINAL TEXT ===\n")
                    f.write(text[:500])
            except Exception:
                pass
            raise RuntimeError(
                "Could not extract clean text from the response. "
                "Raw response saved to debug_raw_response.txt — check it to see "
                "what Duck.ai returned."
            )
